...
export function TvlChart({ history, onRefresh }: TvlChartProps) {
      ...
      {filtered.length < 2 ? (
        <div className="flex flex-col items-center justify-center h-[200px] text-muted-foreground">
          ...
          <p className="text-xs text-muted-foreground/60 mt-1">
            {filtered.length === 0
              ? 'Connect wallet and interact to populate chart'
              : `${filtered.length} point recorded — need at least 2`}
          </p>
          <button
            onClick={handleRefresh}
            disabled={isRefreshing}
            className="mt-3 flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium border bg-muted/30 border-border/50 text-muted-foreground hover:text-foreground transition-all"
          >
